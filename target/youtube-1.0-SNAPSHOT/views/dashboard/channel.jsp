    ...
    <div id="aboutchanel">
        <figure>
            ...
            <div>
                ...
                <span id="sldangki"><c:out value='${userChannel.subscribe == 0 ? 0 : cs.formatNumber(userChannel.subscribe)}'/>
                        &nbsp;người đăng ký</span>
            </div>
        </figure>
        <input type="submit" value="${sService.isSubscribed(userChannel.id, user.id) ? 'ĐÃ ĐĂNG KÝ' : 'ĐĂNG KÝ'}"
               id="btn-subscribe" class="cursor-p ${sService.isSubscribed(userChannel.id, user.id) ? 'subscribed' : ''}"/>
    </div>
    <!-- end aboutchanel -->
